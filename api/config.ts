...
const envSchema = z
  ...
  .superRefine((env, ctx) => {
    ...
    const hasAnyAzureOpenAiConfig = Boolean(
      env.AZURE_OPENAI_API_KEY || env.AZURE_OPENAI_ENDPOINT || env.AZURE_OPENAI_DEPLOYMENT_NAME
    );
      ...
      for (const key of [
        ...
        if (!env[key]) {
          ctx.addIssue({
            ...
            path: [key],
            message:
              'Set AZURE_OPENAI_API_KEY, AZURE_OPENAI_ENDPOINT, and AZURE_OPENAI_DEPLOYMENT_NAME together.',
          });
        }
      }
    }
