...
export const databaseDesignRepository: DatabaseDesignRepository = {
  createDesignSession: async (input) => {
    const [session] = await db.insert(databaseDesignSessions).values(input).returning();
    if (!session) throw new Error('Database design session was not persisted');
    return session;
  },
  createMessages: async (input) => {
    if (input.length === 0) return [];
    return db.insert(databaseDesignMessages).values(input).returning();
  },
  createMigrationRun: async (input) => {
    const [run] = await db.insert(sandboxMigrationRuns).values(input).returning();
    if (!run) throw new Error('Sandbox migration run was not persisted');
    return run;
  },
  createSchemaJson: async (input) => {
    const [schemaJson] = await db.insert(databaseSchemaJsons).values(input).returning();
